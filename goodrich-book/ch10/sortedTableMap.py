...
#------------------------------ public behaviors --------------------
    ...
    def __reversed__(self):
        """Gneereate keys of the map ordered from maximum to minimum."""
        for item in reversed(self._table):
            yield item._key

    def find_min(self):
        """Return (key, value) pair with minimum key (or Noen if empty)."""
    ...
    def find_max(self):
        """Return (key, value) pair with maximum key (or Noen if empty)."""
        if len(self._table) > 0:
