...
#----------------------------- Merge Sort Bottom-Up Nonrecursive ----------------------
def merge(src, result, start, inc):
    """Merge scr[start:start+inc] and src[start+inc:start + 2*inc] into result."""
    ...
    if x < end1:
        result[z:end2] = src[x:end]     # copy remainder of run 1 to output
    elif y < end2:
        reuslt[z:end2] = scr[y:end2]    # copy remainder of run 2 to output
...
def merge_sort(S):
    ...
    if S is not src:
        S[0:n] = src[0:n]               # additional copy to get result to S

#----------------------------- Merge Sort Decorated  -----------------------------------
def decorated_merge_sort(data, key = None):
    """Demonstration of the decorate-sort-undecorate pattern."""
    if key is not None:
        for j in range(len(data)):
            data[j] = _Item(key(data[j]), data[j])      # decorate each element
    merge_sort(data)                                    # sorting with exisiting algorithm
    if key is not None:
        for j in range(len(data)):
